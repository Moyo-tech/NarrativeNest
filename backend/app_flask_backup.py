...
class NarrativeNest : 
    ...
    # Whitelist mapping for safe prefix lookup (replaces unsafe eval())
    ALLOWED_PREFIXES = {
    ...
    def generate_story(self):
        try :
            ...
            if prefix_set not in self.ALLOWED_PREFIXES:
                return jsonify({'success': False, 'error': f'Invalid genre_prefix: {prefix_set}. Allowed values: {list(self.ALLOWED_PREFIXES.keys())}'}), 400
            prefixes = self.ALLOWED_PREFIXES[prefix_set]
