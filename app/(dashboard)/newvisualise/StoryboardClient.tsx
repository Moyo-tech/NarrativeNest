...
const StoryboardClient = () => {
  ...
  useEffect(() => {
    console.log("Saving storyboards to local storage:", storyboards);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(
        API_ENDPOINTS.generateStoryboard,
        { script }
      );
      const storyboard = response.data;
      setStoryboards(storyboard);
    } catch (error) {
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-3 gap-4 p-4">
                {storyboards.map((storyboard, index) => (
                  <div key={index} className="relative flex flex-col mt-6 text-gray-700 bg-white shadow-md bg-clip-border rounded-xl w-80">
...
export default StoryboardClient;
